#if ALL_TESTS
namespace KarambaCommon.Tests.Elements
{
    using System;
    using System.Collections.Generic;
    using System.Diagnostics;
    using System.Drawing.Text;
    using Karamba.CrossSections;
    using Karamba.Elements;
    using Karamba.Geometry;
    using Karamba.Loads;
    using Karamba.Models;
    using Karamba.Supports;
    using Karamba.Utilities;
    using NUnit.Framework;

    [TestFixture]
    public class ModelElementStraightLine_Tests
    {
        [OneTimeSetUp]
        public void CreateTestModel()
        {
            var k3d = new Toolkit();

            var line = new Line3 { PointAtStart = new Point3(0, 0, 0), PointAtEnd = new Point3(10, 0, 0), };

            var logger = new MessageLogger();
            var crossSec = new CroSec_Circle();
            var beam = k3d.Part.LineToBeam(line, "anyId", crossSec, logger, out _);

            var supports = new List<Support>
            {
                k3d.Support.Support(0, k3d.Support.SupportHingedConditions),
                k3d.Support.Support(1, k3d.Support.SupportHingedConditions),
            };

            List<Load> loads = new List<Load>() { k3d.Load.GravityLoad(new Vector3(0, 0, 1)), };

            _model = k3d.Model.AssembleModel(
                beam,
                supports,
                loads,
                out _,
                out _,
                out _,
                out _,
                out _);

            _model = k3d.Algorithms.AnalyzeThI(
                _model,
                out var maxDispls,
                out _,
                out _,
                out _);

            _maxDisplacement = maxDispls[0];
        }

        private Karamba.Models.Model _model;
        private double _maxDisplacement;

        private static void ChangeMaxSectLength(Karamba.Models.Model model, double max)
        {
            model.dp.basic.maxSectLength = max;
        }

        [Test]
        public void NCrossSec_ChangeMaxSectLength_WillChangeSectionNumber()
        {
            // Arrange
            var model = _model.Clone();
            var elementStraightLine = _model.elems[0] as ModelElementStraightLine;
            var length = elementStraightLine.elementLength(model);
            var r = new Random();
            ChangeMaxSectLength(model, r.NextDouble());

            // Act
            var crossSectionCount = elementStraightLine.nCroSec(model);
            var segmentLength = length / (crossSectionCount - 1);
            var segmentLength2 = length / (crossSectionCount - 2);

            // Assert
            Assert.That(segmentLength, Is.Not.GreaterThan(model.dp.basic.maxSectLength).Within(double.Epsilon));
            Assert.That(segmentLength2, Is.GreaterThanOrEqualTo(model.dp.basic.maxSectLength).Within(double.Epsilon));
        }

        [Test]
        public void GetPositionOfCrossSections_ReturnValues_InParametricSpace()
        {
            // Arrange
            var model = _model.Clone();
            var elementStraightLine = model.elems[0] as ModelElementStraightLine;
            double max = 3;
            ChangeMaxSectLength(model, max);

            // Act
            var crossSecsCount = elementStraightLine.nCroSec(model);
            var positions = elementStraightLine.GetCrossSectionsPositions(model);

            // Assert
            Assert.That(crossSecsCount, Is.EqualTo(5));
            CollectionAssert.AreEqual(positions, new List<double> { 0.0, 0.25, 0.5, 0.75, 1.0 });
        }

        [Test]
        public void GetElementState_ValidPositions_ReturnDeformedStates()
        {
            var elementStraightLine = _model.elems[0] as ModelElementStraightLine;
            var position = 0.5;

            var state = elementStraightLine.GetElementState(_model, _model.lc_super_model_results, position);

            Assert.That(state.BasePoint.X, Is.EqualTo(5));
            Assert.That(state.BasePoint.Y, Is.EqualTo(0));
            Assert.That(state.BasePoint.Z, Is.EqualTo(_maxDisplacement).Within(double.Epsilon));
        }

#if TEST_CAN_THROW_EXCEPTION
        [Test]
        public void GetElementState_InvalidPositions_ThrowException()
        {
            // Arrange
            var elementStraightLine = _model.elems[0] as ModelElementStraightLine;
            var position = 1.5;

            // Act + Assert
            try
            {
                var state = elementStraightLine?.GetElementState(_model, _model.lc_super_model_results, position);
                Assert.Fail();
            }
            catch (Exception e)
            {
                Assert.That(e, Is.TypeOf<ArgumentOutOfRangeException>());
            }
        }

#endif

        [Test]
        public void GetElementStates_ValidPositions_ReturnDeformedStates()
        {
            var elementStraightLine = _model.elems[0] as ModelElementStraightLine;
            var position = 0.5;

            var statesCollection = elementStraightLine.GetElementStates(
                _model,
                _model.lc_super_model_results,
                new List<double> { position });
            statesCollection.TryGetStateFromPosition(position, out var state);

            Assert.That(state.BasePoint.X, Is.EqualTo(5));
            Assert.That(state.BasePoint.Y, Is.EqualTo(0));
            Assert.That(state.BasePoint.Z, Is.EqualTo(_maxDisplacement).Within(double.Epsilon));
        }

#if TEST_CAN_THROW_EXCEPTION
        [Test]
        public void GetElementStates_InvalidPositions_ThrowException()
        {
            // Arrange
            var elementStraightLine = _model.elems[0] as ModelElementStraightLine;
            var position = 1.5;

            // Act + Assert
            try
            {
                var statesCollection = elementStraightLine?.GetElementStates(
                    _model,
                    _model.lc_super_model_results,
                    new List<double> { position });
                statesCollection?.TryGetStateFromPosition(position, out var state);

                Assert.Fail();
            }
            catch (Exception e)
            {
                Assert.That(e, Is.TypeOf<ArgumentOutOfRangeException>());
            }
        }
#endif
    }
}
#endif